...
def callback(ros_image):
    ...
    if len(averaged_lines) > 0:
        ...
        if (frame % 3 == 0):
            img_count += 1
            cv2.imwrite('dataset/image_' + str(img_count) + '_' + posneg(steering_error) + '_' + steeringerror(
                steering_error) + '.jpg', cv_image)
...
def main():
    ...
    img_sub = rospy.Subscriber("/catvehicle/camera_front/image_raw_front", Image, callback)
