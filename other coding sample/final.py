...
class Follower:
    ...
    def image_callback(self, msg):
        ...
        upper_black = numpy.array([355, 55, 100])
        ...
        masky = cv2.inRange(hsv, lower_black, upper_black)
        # remove pixels not in this range
        mask_yw = cv2.bitwise_or(maskw, masky)
        rgb_yw = cv2.bitwise_and(image, hsv, mask=mask_yw).astype(numpy.uint8)
        ...
        rgb_yw2[0:int(search_top), 0:w] = 0
        rgb_yw2[int(search_bot):h, 0:w] = 0
        ...
        if M['m00'] > 0:
            ...
            self.twist.angular.z = (-float(err) / 100) * 2.5 + ((err - perr) / (
                    rospy.get_time() - ptime)) * 1 / 50 / 100 + (serr * dt) * 1 / 20 / 100  # 1 is best, starting 3
            # unstable
            serr = err + serr
            perr = err
        ...
        resized_mask = cv2.resize(rgb_yw2, (620, 360))

        cv2.imshow("masked image", resized_mask)
